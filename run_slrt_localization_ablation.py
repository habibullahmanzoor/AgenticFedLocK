"""SLRT localization ablation: per-region cosine deviation vs. whole-model cosine.

This paper's mechanism section describes SLRT as computing root-cosine
deviation separately per security-relevant region (shared-core, classifier)
and taking the worst region, rather than a single whole-model cosine like
plain FLTrust -- with the stated rationale that a concentrated attack signal
would otherwise be diluted by thousands of unaffected parameters. That
rationale was argued narratively but never tested directly. This ablation
tests it: does region-localization actually change the outcome, or would a
whole-model comparison have done just as well inside this framework?

`slrt_localization_mode=per_region` is the standing default and already
exists as `cifar10_seed{N}_agentlock` -- reused, not rerun. Only
`whole_model` needs fresh runs.
"""
...
COMMON_ARGS = [
    ...
    "--slrt-localization-mode", "whole_model",
]


def run_one(seed: int) -> None:
    out_dir = RUNS_DIR / f"cifar10_seed{seed}_slrt_whole_model"
...
def main() -> None:
    ...
    for seed in args.seeds:
        run_one(seed)
    print(f"=== slrt-localization ablation complete (seeds={args.seeds}) ===", flush=True)
